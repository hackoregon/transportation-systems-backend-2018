...
class PassengerCensusListEndpointsTestCase(TestCase):
    ...
    def test_list_200_response(self):
        response = self.client.get('/transportation-systemspassenger_census/PassengerCensus/')
...
class PassengerCensusRoutesAnnualEndpointTestCase(TestCase):
    ...
    def test_total_stops_response(self):
        response = self.client.get('/transportation-systemspassenger_census/PassengerCensusRoutesAnnual/?route=1&year=2002')
        assert response.status_code == 200
    ...
    def test_missing_route(self):
        response = self.client.get('/transportation-systemspassenger_census/PassengerCensusRoutesAnnual/?year=2002')
        assert response.status_code == 400
        self.assertEqual(response.data, 'Missing Route Number paramater')
    def test_missing_year(self):
        response = self.client.get('/transportation-systemspassenger_census/PassengerCensusRoutesAnnual/?route=1')
        assert response.status_code == 400
        self.assertEqual(response.data, 'Must include a year for search')
    def test_nonexistent_route(self):
        response = self.client.get('/transportation-systemspassenger_census/PassengerCensusRoutesAnnual/?route=678&year=2002')
        assert response.status_code == 404
        self.assertEqual(response.data, 'Route Number not found')
    def test_nonexistent_route(self):
        response = self.client.get('/transportation-systemspassenger_census/PassengerCensusRoutesAnnual/?route=1&year=2990')
        assert response.status_code == 404
